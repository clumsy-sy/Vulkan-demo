...
auto CommandManager::createCommandPool() -> vk::CommandPool {
  auto &app = Application::GetInstance();

  vk::CommandPoolCreateInfo createInfo;

  createInfo.setQueueFamilyIndex(app.queueFamilyIndices.graphicQueue.value())
      .setFlags(vk::CommandPoolCreateFlagBits::eResetCommandBuffer);

  return app.device.createCommandPool(createInfo);
}

auto CommandManager::CreateCommandBuffers(std::uint32_t count)
...
auto CommandManager::CreateOneCommandBuffer() -> vk::CommandBuffer {
  return CreateCommandBuffers(1)[0];
}

void CommandManager::FreeCmd(const vk::CommandBuffer &cmdBuf) {
  Application::GetInstance().device.freeCommandBuffers(pool_, cmdBuf);
}
