...
void Texture::init(void *data, uint32_t w, uint32_t h) {
  ...
  std::unique_ptr<BufferPkg> buffer(new BufferPkg(size, vk::BufferUsageFlagBits::eTransferSrc,
      vk::MemoryPropertyFlagBits::eHostCoherent | vk::MemoryPropertyFlagBits::eHostVisible));
  memcpy(buffer->map, data, size);

  createImage(w, h);
  allocMemory();
  Application::GetInstance().device.bindImageMemory(image, memory, 0);
...
void Texture::createImage(uint32_t w, uint32_t h) {
  ...
  image = Application::GetInstance().device.createImage(createInfo);
}
...
void Texture::transformData2Image(BufferPkg &buffer, uint32_t w, uint32_t h) {
  Application::GetInstance().commandManager->ExecuteCmd(Application::GetInstance().graphicQueue, [&](vk::CommandBuffer cmdBuf) {
    vk::BufferImageCopy region;
    vk::ImageSubresourceLayers subsource;
    subsource.setAspectMask(vk::ImageAspectFlagBits::eColor).setBaseArrayLayer(0).setMipLevel(0).setLayerCount(1);
...
void Texture::transitionImageLayoutFromUndefine2Dst() {
  Application::GetInstance().commandManager->ExecuteCmd(Application::GetInstance().graphicQueue, [&](vk::CommandBuffer cmdBuf) {
    ...
    range.setLayerCount(1).setBaseArrayLayer(0).setLevelCount(1).setBaseMipLevel(0).setAspectMask(
    ...
    cmdBuf.pipelineBarrier(
        vk::PipelineStageFlagBits::eTopOfPipe, vk::PipelineStageFlagBits::eTransfer, {}, {}, nullptr, barrier);
  });
}
...
void Texture::transitionImageLayoutFromDst2Optimal() {
  Application::GetInstance().commandManager->ExecuteCmd(Application::GetInstance().graphicQueue, [&](vk::CommandBuffer cmdBuf) {
    ...
    barrier.setImage(image)
        ...
        .setSubresourceRange(range);
    cmdBuf.pipelineBarrier(
        vk::PipelineStageFlagBits::eTransfer, vk::PipelineStageFlagBits::eFragmentShader, {}, {}, nullptr, barrier);
  });
}
...
void Texture::createImageView() {
  ...
  createInfo.setImage(image)
      ...
      .setFormat(vk::Format::eR8G8B8A8Srgb)
      .setSubresourceRange(range);
  view = Application::GetInstance().device.createImageView(createInfo);
}
...
void Texture::updateDescriptorSet() {
  ...
  imageInfo.setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal)
      .setImageView(view)
      .setSampler(Application::GetInstance().sampler);
  ...
  Application::GetInstance().device.updateDescriptorSets(writer, {});
}
